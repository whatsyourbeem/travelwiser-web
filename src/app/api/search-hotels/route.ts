...
export async function GET(request: Request) {
  ...
  try {
    ...
    let matchedHotels: Hotel[];

    if (!name) {
      // 검색어가 없으면, 호텔 목록의 일부를 기본값으로 반환합니다.
      // 데이터베이스에서 무작위로 샘플링하거나, 특정 기준(예: 인기순)으로 정렬할 수 있습니다.
      // 여기서는 간단하게 처음 20개 호텔을 반환합니다.
      matchedHotels = allHotels.slice(0, 20);
    } else {
      // 검색어(name)를 포함하는 호텔을 필터링합니다. (대소문자 무시)
      const lowercasedName = name.toLowerCase();
      matchedHotels = allHotels
        .filter((hotel) => {
          const translatedNameMatch = hotel.hotel_translated_name
            ?.toLowerCase()
            .includes(lowercasedName);
          const nameMatch = hotel.hotel_name
            ?.toLowerCase()
            .includes(lowercasedName);
          return translatedNameMatch || nameMatch;
        })
        .slice(0, 20); // 검색 결과를 20개로 제한합니다.
    }
